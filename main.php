...
class WebSecurityScanner {
    ...
    public function __construct() {
        $this->config = [
            'access_key' => 'secure_scanner_2024',
            'max_file_size' => 10485760, // 10MB
            'scan_extensions' => ['php', 'phtml', 'shtml', 'php7', 'phar', 'php58', 'haxor', 'alfa'],
     ...
     */
    public function generateReport($scanDirectory = '') {
        ...
        <p class="text-dark-400 mt-2">Professional security scanner for web servers</p>
        </header>
        ...
        <label for="scan_dir" class="block text-sm font-medium mb-2">Enter directory path to scan</label>
        <input
        type="text"
        ...
        <p>Enter a directory path above and click "Start Scan" to begin.</p>
        </div>
        <?php endif; ?>
        <?php endif; ?>
...
if (isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'get_file_content') {
        if (!isset($_POST['key']) || $_POST['key'] !== 'secure_scanner_2024') {
...
// Verify access
if (!isset($_GET['key']) || $_GET['key'] !== 'secure_scanner_2024') {
    http_response_code(404);
...
echo $scanner->generateReport($scanDirectory);
?>
